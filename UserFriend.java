...
public class UserFriend {
    ...
    public static Observable<UserFriend> getFriends(int userId) {
        return Observable.fromArray(
                new UserFriend(userId, getRandomFriendId(userId)),
                new UserFriend(userId, getRandomFriendId(userId)),
                new UserFriend(userId, getRandomFriendId(userId))
        );
    }
    private static int getRandomFriendId(int userId) {
        int friendId;
        do {
            friendId = (int) (Math.random() * 100);
        } while (friendId == userId); // Избегаем совпадения friendId с userId
        return friendId;
    }
