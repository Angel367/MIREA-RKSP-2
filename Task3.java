...
public class Task3 {
    ...
    public Task3(String fileType, int fileSize) {
...
class FileGenerator implements Runnable {
    private BlockingQueue<Task3> queue;
    public FileGenerator(BlockingQueue<Task3> queue) {
        this.queue = queue;
    }
    @Override
    public void run() {
        ...
        while (true) {
            try {
                ...
                Task3 file = new Task3(randomFileType, randomFileSize);
...
class FileProcessor implements Runnable {
    ...
    public FileProcessor(BlockingQueue<Task3> queue, String allowedFileType) {
        this.queue = queue;
    ...
    public void run() {
        while (true) {
            try {
                Task3 file = queue.take(); // Получаем файл из очереди
